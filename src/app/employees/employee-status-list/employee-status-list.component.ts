...
export class EmployeeStatusListComponent {
  ...
  signIn() {
    if (this.barId) {
      this.dialog
        .open(EmployeClockerDialogComponent)
        .afterClosed()
        .pipe(
          filter((login): login is EmployeeLogin => !!login),
          switchMap(login =>
            this.checkIn({ ...login, barId: this.barId as string })
          )
        )
        .subscribe(
          () => this.sbs.openSuccess('Employee checked in successfully!', 3000),
    ...
    if (this.barId) {
      this.dialog
        ...
        .pipe(
          filter((login): login is EmployeeLogin => !!login),
          switchMap(login =>
            this.checkOut({ ...login, barId: this.barId as string })
          )
        )
        .subscribe(
          () =>
            this.sbs.openSuccess('Employee checked out successfully!', 3000),
          error => this.sbs.openError(error.message, 3000)
        );
    }
  }
